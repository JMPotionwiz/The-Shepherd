...
public class main {
    ...
    public static String ERRAH = "", n_009 = "";
    ...
    public static void main(String[] args) {
        try{
            if (saves.exists()) {
                Scanner r = new Scanner(saves);
                ArrayList<String> in = new ArrayList<String>();
                while (r.hasNextLine()) {
                    in.add(r.nextLine());
                }
                int l = 0;
                Difficulty = (byte) Integer.parseInt(in.get(l++));
                MapSize = (byte) Integer.parseInt(in.get(l++));
                int E_001 = keys.length;
                for (int i = 0; i < E_001; i++) {
                    keys[i].changeKeybind(in.get(l++));
                }
                highScore = Long.parseLong(in.get(l++));
                alwaysShowHealthbars = Boolean.parseBoolean(in.get(l++));
                if (in.size() > l) n_009 = in.get(l++);
            } else {
                saves.createNewFile();
        ...
        while (true) {
            ...
            if (buttons.buttonsInUse) buttons.buttonHandler();
            if (keys[7].pressed()) {
    ...
    public static class menus {
        public static void MainMenu(Boolean Ending) { //menu = 1
            ...
            buttons.buttons.add(tmp);
            tmp = new ArrayList<Button>();
            String[] d1 = {"Easy","Normal","Hard","NIGHTMARE"};
