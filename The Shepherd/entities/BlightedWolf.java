...
public class BlightedWolf extends WolfBase {
    public BlightedWolf(double x, double y) {
        super(x,y);
        this.health = 40;
        this.maxHealth = 40;
        this.healing = 0;
    ...
    public void tick() {
        if (main.maintool.wolfSpawnZone(this.x, this.y) && (this.attacked || this.PANIC || this.stopHoming)) {
            ...
            this.blighted = 100;
            super.tick();
            if (this.attackCooldown == 0 && this.health > 0) this.attack(this.bB, 15d + main.wolfAttack);
        }
    }
    public void control() {
        if (tool.getDistance(this.x, this.y, target.getX(), target.getY()) <= 48) this.stopHoming = true;
        
        if (!this.attacked && !this.stopHoming && this.target != null) {
    ...
    public void render(Graphics2D g) {
        ...
        g.drawImage(main.bwolfA.get(i.toString()), (int)Math.round(this.x)-12, (int)Math.round(this.y)-15, null);
